 ...
 * Time: 15:33
 */
class DevsqlController extends AbstractController
{
  ...
  public function shopinfoAction()
  {
    ...
    $db = Phalcon\DI::getDefault()->get("devssw_database");
    $res = $db->fetchOne("SELECT * FROM shopify_ssw_client where client_id = {$client_id};");
    exit(json_encode([
      "shop" => $res
    ]));
  }
  ...
  public function clientSettingsAction()
  {
    ...
    $result_errors = array();
    $settings = [];
    $maindb = Phalcon\DI::getDefault()->get('devssw_database');
    $res = $maindb->fetchOne("SELECT * FROM shopify_ssw_client where client_id = {$client_id};");
    if ($res) {
      if ($res['db_id'] == 3) {
        $dbAdapter = 'devssw_database2';
      } else if ($res['db_id'] == 2) {
        $dbAdapter = 'devssw_database1';
      } else if ($res['db_id'] == 0) {
        $dbAdapter = 'devssw_database';
      }
      try {
        $db = Phalcon\DI::getDefault()->get($dbAdapter);
        if (!empty($filter_text)) {
          $filter_text = addslashes($filter_text);
          $sql = "SELECT * from {$client_id}_shopify_core_setting where name like '%{$filter_text}%' limit 200;";
        } else
  ...
  public function authAction()
  {
    ...
    if ($this->request->isPost()) {
      if (Core::authorize($_POST['secret_key'], 'sql')) {
        return $this->dispatcher->forward(array('controller' => 'sql', 'action' => 'db'));
      } else {
        $error = true;
      }
  ...
  public function dbResultAction()
  {
    ...
    //check select without limit, update and delete without WHERE clause
    ...
    if (!$stop) {
      if ($db == 'crm') {
        ...
        $dbAdapter = 'staging_database';
      } else if ($db == 'testing') {
        $dbAdapter = 'testing_database';
      } else if ($db == 'temp1_database' || $db == 'temp2_database') {
        $dbAdapter = $db;
      } else if (strstr($db, 'db6') !== false) {
        $dbAdapter = $db;
      } else {
        $dbAdapter = 'devssw_database' . $db;
      }
      /**
       * @var Phalcon\Db\Adapter\Pdo\Mysql $db
       */
  ...
  private function getDatabaseCache($new = false, $crm = false)
  {
    if ($new) {
      /**
       * @var Phalcon\Db\Adapter\Pdo\Mysql $db
  ...
  private function dbGetDBAdapter($dbName)
  {
    $dbAdapter = 'db';
    $dbName = trim($dbName);
    if (strstr($dbName, 'shopify_dev') !== false) {
      $dbAdapter = str_replace('shopify_dev', 'devssw_database', $dbName);
    } elseif ($dbName == 'crm_ssw' || $dbName == 'crm_ssw') {
      $dbAdapter = 'db';
    }
